...
class VMCodeWriter:
    ...
    def writeArithmetic(self, line: int, command: str) -> None:
        """
        Write the assembly code for the arithmetic command.
        """
        if command not in ['add', 'sub', 'neg', 'eq', 'gt', 'lt', 'and', 'or', 'not']:
            raise SyntaxError(f"Unknown arithmetic command: '{command}' at line {line}")
    ...
    def writePushPop(self, line: int, command: str, segment: str, index: int) -> None:
        ...
        if command not in ['C_PUSH', 'C_POP']:
            raise SyntaxError(f"Unknown command type for push/pop: '{command}' at line {line}")
